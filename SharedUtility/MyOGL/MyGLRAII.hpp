...
// 忌々しいことこのうえない。
// OpenGL 4.5 のコア機能として昇格した GL_ARB_direct_state_access (Direct State Access) にて、
// バッファやテクスチャに関してはようやくオブジェクト指向できるようになったが、GL コンテキストは相変わらずダメ。
...
// Direct3D はバージョンアップのたびに Side-by-Side で共存できるように API が増築あるいは新築されるので、
// 既存の API はそのままで、新しい API で思い切った設計変更ができるのも強み。
